...
async function seedRegistrationsIfEmpty() {
  try {
    ...
    console.log('Registry table is empty. Auto-seeding from emails_test.xlsx...');
    ...
    if (records.length > 0) {
      await ExaminerRegistration.bulkCreate(records, { ignoreDuplicates: true });
      console.log(`Successfully seeded ${records.length} registrations from emails_test.xlsx!`);
    }
  } catch (err) {
